...
To create the endpoint, a scoring script- one that loads the model and handle inferencing requests- must be supplied. (Side note, Azure ML uses Flask as the default web app framework.) 
...
All the above code is stock-standard from the SDK examples BUT THERE'S ONE IMPORTANT ENTITY TO SET HERE THAT IS NOT IMMEDIATELY OBVIOUS:
...
In the image, you may have noticed two deployments in the one endpoint, that is the result of multiple deployment attempts. Make sure you set the traffic to the right deployment to test if you are in the same situation. 
